"""Enterprise authorization wrapper for upstream MCP routes."""
...
from datus.api.routes import mcp_routes as upstream_mcp_routes
...
async def _require_mcp_server_add_permission(ctx: McpModuleCtx) -> None:
...
async def _require_mcp_server_list_permission(ctx: McpModuleCtx) -> None:
    await _require_mcp_permission(
        ctx,
        "mcp.server.list",
        resource_type="mcp_server",
...
async def _require_mcp_server_remove_permission(server_name: str, ctx: McpModuleCtx) -> None:
    await _require_mcp_permission(
        ctx,
        "mcp.server.remove",
        resource_type="mcp_server",
...
async def _require_mcp_server_edit_permission(server_name: str, ctx: McpModuleCtx) -> None:
    await _require_mcp_permission(
        ctx,
        "mcp.server.edit",
        resource_type="mcp_server",
...
async def _require_mcp_server_connectivity_permission(server_name: str, ctx: McpModuleCtx) -> None:
    await _require_mcp_permission(
        ctx,
...
async def _require_mcp_server_tools_permission(server_name: str, ctx: McpModuleCtx) -> None:
    await _require_mcp_permission(
        ctx,
        "mcp.server.tools",
        resource_type="mcp_server",
...
async def _require_mcp_filter_view_permission(server_name: str, ctx: McpModuleCtx) -> None:
    await _require_mcp_permission(
        ctx,
        "mcp.filter.view",
...
async def _require_mcp_filter_set_permission(server_name: str, ctx: McpModuleCtx) -> None:
    await _require_mcp_permission(
        ctx,
        "mcp.filter.set",
        resource_type="mcp_filter",
...
async def _require_mcp_filter_remove_permission(server_name: str, ctx: McpModuleCtx) -> None:
    await _require_mcp_permission(
        ctx,
        "mcp.filter.remove",
        resource_type="mcp_filter",
...
async def list_servers(
    svc: ServiceDep,
    _ctx: McpModuleCtx,
    server_type: Optional[str] = upstream_mcp_routes.SERVER_TYPE_QUERY,
) -> Result[Dict[str, Any]]:
    return await upstream_mcp_routes.list_servers(svc, server_type)
...
async def add_server(
    ...
    return await upstream_mcp_routes.add_server(server_config, svc)
...
async def remove_server(
    svc: ServiceDep,
    _ctx: McpModuleCtx,
    server_name: str = upstream_mcp_routes.REMOVE_SERVER_NAME_PATH,
) -> Result[Dict[str, Any]]:
...
async def get_tool_filter(
    ...
    return await upstream_mcp_routes.get_tool_filter(svc, server_name)
...
async def set_tool_filter(
    ...
    svc = await api_deps.resolve_datus_service_for_request(http_request)
    return await upstream_mcp_routes.set_tool_filter(filter_config, svc, server_name)
...
async def remove_tool_filter(
    ...
    return await upstream_mcp_routes.remove_tool_filter(svc, server_name)
