"""Enterprise success-story persistence extensions."""
...
from datus.api.models.downstream import SuccessStoryData, SuccessStorySource
...
class EnterpriseSuccessStoryService(SuccessStoryService):
    """Persist canonical success stories with datasource isolation and idempotency."""
    ...
    def save(self, source: SuccessStorySource) -> SuccessStoryData:
        """Persist a server-resolved execution, returning stable story metadata."""
        subagent_name = source.subagent_name
        target_dir = self._resolve_enterprise_target_dir(source.datasource_id, subagent_name)
    ...
    def migrate_legacy_file(
        ...
        *,
        ...
        target_dir = self._resolve_enterprise_target_dir(normalized_datasource, normalized_subagent)
        target_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _resolve_enterprise_target_dir(self, datasource_id: str, subagent_name: str) -> Path:
        """Compute a safe per-datasource/per-subagent directory."""
    ...
    def _read_canonical_rows(csv_path: Path, datasource_id: str) -> list[dict[str, str | None]]:
        ...
        with open(csv_path, newline="", encoding="utf-8-sig") as source_file:
            ...
            for index, raw in enumerate(reader):
                ...
                source_id = raw.get("source_id") or EnterpriseSuccessStoryService._legacy_source_id(raw, index)
                rows.append(
                    {
