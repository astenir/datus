...
BI 运行时配置统一写在 `agent.yml` 的 `agent.services.bi_tools` 下。
...
在 `agent.services.bi_tools` 下配置 BI 平台：
...
- `bi_platform` 用来从 `services.bi_tools` 中选择一个平台配置。
...
- `services.bi_tools` 是 BI 凭据的唯一运行时来源。
- 敏感值支持 `${ENV_VAR}` 环境变量替换。

## 相关文档

- [BI 工具配置](../configuration/bi_tools.md)
