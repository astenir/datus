...
"""Unit tests for the enterprise configuration routes."""
...
from datus_enterprise.api import config_routes
from datus_enterprise.api.config_routes import (
...
def test_create_app_registers_authoritative_config_routes_once():
    ...
    expected = {
        ...
        ("POST", "/api/v1/config/models/test"),
        ("POST", "/api/v1/config/models/test-saved"),
        ("POST", "/api/v1/config/datasources/test"),
        ("POST", "/api/v1/config/datasources/test-saved"),
    }
    ...
    for method, path in expected:
        ...
        assert len(routes) == 1
        assert routes[0].endpoint.__module__ == "datus_enterprise.api.config_routes"
