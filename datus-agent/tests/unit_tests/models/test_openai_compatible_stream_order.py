...
class TestRawEventEarlyCapture:
    ...
    Reasoning streams as thinking_delta while normal output streams as response_delta.
    """

    @pytest.mark.asyncio
    async def test_text_deltas_yield_response_delta_actions(self):
        """Normal output deltas yield response_delta actions with accumulated text."""
        events = [
        ...
        assert assistant_actions[0].output["content_type"] == "markdown"
    ...
    async def test_deltas_before_tool_call_yields_thinking_then_processing(self):
        ...
        non_delta = [a for a in actions if a.action_type not in {"thinking_delta", "response_delta"}]
    ...
    @pytest.mark.asyncio
    async def test_response_delta_stream_id_shared(self):
        """All response_delta actions in one stream share the same action_id."""
        events = [
