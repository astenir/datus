...
class TestPromptManager:
    ...
    def test_render_template_uses_request_scoped_agent_prompt_content(self, tmp_path):
        path_manager = DatusPathManager(tmp_path / "tenant_home")
        ...
        assert manager.render_template("chat_custom_system", "1.0", audience="analysts") == (
            "Custom prompt for analysts"
        )

    def test_request_scoped_agent_prompt_does_not_leak_between_configs(self, tmp_path):
        path_manager = DatusPathManager(tmp_path / "tenant_home")
        ...
        assert get_prompt_manager(agent_config=first_config).render_template("chat_custom_system", "1.0") == (
            "First request"
        )
