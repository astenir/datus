...
from datus.prompts.prompt_manager import (
    PromptManager,
    capture_prompt_template_identities,
    get_prompt_manager,
)
...
class TestPromptManager:
    ...
    def test_template_identity_tracks_resolved_version_source_and_content(self, tmp_path):
        path_manager = DatusPathManager(tmp_path / "tenant_home")
        manager = _make_manager(tmp_path, path_manager=path_manager)
    ...
    def test_template_identity_includes_static_jinja_dependencies(self, tmp_path):
        path_manager = DatusPathManager(tmp_path / "tenant_home")
        manager = _make_manager(tmp_path, path_manager=path_manager)
    ...
    def test_render_template_reloads_user_override_for_cached_include(self, tmp_path):
        path_manager = DatusPathManager(tmp_path / "tenant_home")
        manager = _make_manager(tmp_path, path_manager=path_manager)
        _write_template(manager.default_templates_dir, "greet", "1.0", "{% include '_shared.j2' %}")
    ...
    def test_capture_records_actual_runtime_template_resolution(self, tmp_path):
        path_manager = DatusPathManager(tmp_path / "tenant_home")
    ...
    def test_capture_records_raw_template_resolution(self, tmp_path):
        path_manager = DatusPathManager(tmp_path / "tenant_home")
        manager = _make_manager(tmp_path, path_manager=path_manager)
