...
class ErrorCode(Enum):
    ...
    COMMON_UNSUPPORTED = (
        "100004",
        "Unsupported value `{your_value}` for field `{field_name}`",
    )
    ...
    DB_CONNECTION_FAILED = (
        "500001",
        "Failed to establish connection to database. Error details: {error_message}",
    )
    DB_CONNECTION_TIMEOUT = (
        "500002",
        "Connection to database timed out. Error details: {error_message}",
    )
    ...
    DB_TRANSACTION_FAILED = (
        "500009",
        "Database transaction failed. Error details: {error_message}",
    )
