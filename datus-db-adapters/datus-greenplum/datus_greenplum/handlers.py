...
"""URI builder and context resolver for Greenplum."""
...
def build_greenplum_uri(db_config) -> str:
    """Build a SQLAlchemy connection URI for Greenplum.

    Greenplum uses the PostgreSQL wire protocol, so we use the
...
def resolve_greenplum_context(db_config, uri: str) -> Tuple[str, str, str, str]:
    """Resolve connection context from URI and config.

    Returns:
        Tuple of (db_type, catalog, database, schema)
    """
    url = make_url(uri)
    ...
    return "greenplum", "", database, schema
