...
@pytest.mark.integration
def test_tpch_get_columns(tpch_setup: SparkConnector):
    """Test getting column schema for tpch_customer table."""
    columns = tpch_setup.get_schema(database_name="default", table_name="tpch_customer")
...
@pytest.mark.integration
def test_tpch_get_columns_nation(tpch_setup: SparkConnector):
    """Test getting column schema for tpch_nation table."""
    columns = tpch_setup.get_schema(database_name="default", table_name="tpch_nation")
    column_names = {col["name"] for col in columns}
    assert "nationkey" in column_names
    assert "name" in column_names
...
def test_tpch_query_region(tpch_setup: SparkConnector):
    """Test querying tpch_region - should have 5 regions."""
    result = tpch_setup.execute(
        {"sql_query": "SELECT * FROM `default`.`tpch_region`"},
        result_format="list",
    )
...
@pytest.mark.integration
@pytest.mark.acceptance
def test_tpch_query_nation(tpch_setup: SparkConnector):
    """Test querying tpch_nation - should have 25 nations."""
    result = tpch_setup.execute(
        {"sql_query": "SELECT * FROM `default`.`tpch_nation`"},
        result_format="list",
    )
    assert result.success
...
@pytest.mark.integration
def test_tpch_query_aggregation(tpch_setup: SparkConnector):
    """Test aggregation: count nations per region."""
    result = tpch_setup.execute(
        {
            "sql_query": (
                "SELECT r.name AS region_name, COUNT(n.nationkey) AS nation_count "
                "FROM `default`.`tpch_region` r "
                "JOIN `default`.`tpch_nation` n ON r.regionkey = n.regionkey "
                "GROUP BY r.name "
                "ORDER BY r.name"
            )
        },
...
@pytest.mark.integration
def test_tpch_query_customer_orders(tpch_setup: SparkConnector):
    ...
    result = tpch_setup.execute(
        {
            "sql_query": (
                ...
                "FROM `default`.`tpch_customer` c "
                "JOIN `default`.`tpch_orders` o ON c.custkey = o.custkey "
                "GROUP BY c.name "
