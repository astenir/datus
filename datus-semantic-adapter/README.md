...
| `datus-semantic-osi-engine` | 原生 Rust OSI Engine 适配；在 workspace 外隔离测试 | [README](./datus-semantic-osi-engine/README.md) |
...
运行 workspace 级测试前使用锁文件同步全部包。`datus-semantic-osi-engine` 因未发布的可选 Rust wheel 保持在 workspace 外，其 unit tests 使用 fake binding 隔离运行。修改 MetricFlow 集成时，还应在 `metricflow/` 中运行受影响的 SQL engine/client 测试。
